...
import { useUpdate, getProbableKeys, getUniqueStrings, getUniqueTopLevelKeys, cn, groupArrayBy } from "./utils";
...
import ZrchV2 from "./ZrchV2"


export const v2 = ZrchV2
...
//! ========================================================================================================================
const Zrch:React.FunctionComponent<any> = ({
  defaultOpen = false,
  searchable = [],
  recommended = [],
  searchKeys = [],
  ...
  NoSearchComponent?: () => ReactElement;
  NoResultsComponent?: () => ReactElement;
  FooterComponent?: () => ReactElement;
  RenderItem?:  ({ result, index }:{ result: FuseResult<T>, index: number }) => ReactElement;
  RenderList?:  ({ results, value, onSelect }:{ results: FuseResult<T>[],  groupedResults: [string, FuseResult<T>[]][], value: string, onSelect: (value:any) => void }) => ReactElement;
  ...
  const [ctx, setCtx] = useState<zrchCtx<T>>({
    ...
    isWindowOpen: defaultOpen ? true : false,
    searchable,
    recommended,
    searchKeys,
  });
  ...
  useUpdate(() => {
    ...
    let groupedRes = groupBy 
      ? Object.entries(groupArrayBy(res, (i:any) => i?.item?.[groupBy]  )) 
      : []
