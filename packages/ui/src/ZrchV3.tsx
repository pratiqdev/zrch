...
const Zrch: React.FC<ZrchProps> = (props) => {
  ...
  const performSearch =  React.useMemo(() =>
    _.debounce((fuseRef, value, limit) => {

      console.log('performing search:', value)
      // setSearchResults(value ? fuseRef?.current?.search(value, { limit }) ?? [] : [])
      return value ? fuseRef?.current?.search(value, { limit }) ?? [] : []
  ...
  React.useEffect(() => {
    ...
    const results = performSearch(fuseRef, searchValue, maxResults)
